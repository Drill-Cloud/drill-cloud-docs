...
- [Локальный контур](docs/local-development.md) — полный запуск Node-RED → MQTT → ingest → cloud → UI с серверным SSO.
- [Проверка системы](docs/testing.md) — короткая ручная проверка и запуск автоматизированного smoke-набора.
- [Сервисы и переменные окружения](docs/services.md) — назначение репозиториев, порты и минимальная конфигурация.
